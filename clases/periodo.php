...
function search(){
    ...
    // Query para leer uno solo
    $query = "SELECT  * FROM  " . $this->table_name . " WHERE  indicadores_idindicadores = ? AND territorios_codigo_dane = ? ";
    // AND territorios_codigo_dane = ?
    // Preparar sentencia
    $stmt = $this->conn->prepare( $query ); 
    $stmt->bindParam(1, $this->indicadores_idindicadores);
    $stmt->bindParam(2, $this->territorios_codigo_dane);
